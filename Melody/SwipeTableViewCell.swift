...
//        guard isEditing == false else { return }
    ...
    func showActionsView(for orientation: SwipeActionsOrientation) -> Bool {
        guard let tableView = tableView,
            ...
            let actions = swipeDelegate?.tableView(tableView, editActionsForRowAt: indexPath, for: orientation),
            actions.count > 0
            else {
                return false
        }
...
//        selectedIndexPaths?.forEach { tableView.deselectRow(at: $0, animated: false) }
    ...
    func configureActionsView(with actions: [SwipeAction], for orientation: SwipeActionsOrientation) {
        ...
        let options = swipeDelegate?.tableView(tableView, editActionsOptionsForRowAt: indexPath, for: orientation) ?? SwipeTableOptions()
    ...
    func notifyEditingStateChange(active: Bool) {
        ...
        if active {
            swipeDelegate?.tableView(tableView, willBeginEditingRowAt: indexPath, for: actionsView.orientation)
        } else {
            swipeDelegate?.tableView(tableView, didEndEditingRowAt: indexPath, for: actionsView.orientation)
        }
    }
...
extension SwipeTableViewCell: SwipeActionsViewDelegate {
    ...
    func performFillAction(action: SwipeAction, fillOption: SwipeExpansionStyle.FillOptions) {
        ...
        action.completionHandler = { [weak self] style in
            ...
            self?.swipeDelegate?.tableView(tableView, didEndEditingRowAt: indexPath, for: actionsView.orientation)
