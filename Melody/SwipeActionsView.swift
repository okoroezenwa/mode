...
class SwipeActionsView: MELBorderView {
    ...
    var expansionDelegate: SwipeExpanding? {
        return options.expansionDelegate ?? ScaleAndAlphaExpansion.default//(expandableAction?.hasBackgroundColor == false ? ScaleAndAlphaExpansion.default : nil)
    }
...
//        backgroundColor = options.backgroundColor ?? Themer.borderViewColor()//#colorLiteral(red: 0.862745098, green: 0.862745098, blue: 0.862745098, alpha: 1)
        
        buttons = addButtons(for: self.actions, withMaximum: maxSize)
        layer.cornerRadius = maxSize.height / 2
    }
