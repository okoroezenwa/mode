...
public class SwipeAction: NSObject {
    ...
    public var highlightedBackgroundColor: UIColor? = Themer.borderViewColor()
    ...
    */
    public init(style: SwipeActionStyle, title: String?, handler: ((SwipeAction, IndexPath) -> Void)?) {
        ...
        notifier.addObserver(self, selector: #selector(updateHighlightedColour), name: .themeChanged, object: nil)
    }
    
    public convenience init(style: SwipeActionStyle, title: String?, handler: ((SwipeAction, IndexPath) -> Void)?, image: UIImage?) {
    ...
    @objc private func updateHighlightedColour() {
        
        highlightedBackgroundColor = Themer.borderViewColor()
    }
