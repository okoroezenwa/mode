...
class TableDelegate: NSObject, Detailing {
    ...
    func getActionDetails(from action: SongAction, indexPath: IndexPath, vc: UIViewController?, useAlternateTitle alternateTitle: Bool = false) -> (details: ActionDetails, action: SongAction)? {
        
        guard let container = container, let vc = vc else { return nil }
        
        return (container.singleItemActionDetails(for: action, entity: entityType(), using: container.getEntity(at: indexPath, filtering: container.filterContainer != nil), from: vc, useAlternateTitle: alternateTitle), action)
    }
}
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath, for orientation: SwipeActionsOrientation) -> [SwipeAction]? {
        ...
        case .left:
            ...
            if musicPlayer.nowPlayingItem != nil, let cell = tableView.cellForRow(at: indexPath) as? SongTableViewCell {
                ...
                let actionInfo = getActionDetails(from: .queue(name: cell.nameLabel.text, query: query(at: indexPath, filtering: container.filterContainer != nil)), indexPath: indexPath, vc: container.filterContainer ?? container as? UIViewController)
                
                let queue = SwipeAction.init(style: .default, title: actionInfo?.details.title.lowercased(), handler: { _, _ in actionInfo?.details.handler() })
                
                queue.image = actionInfo?.action.icon
        ...
        case .right:
            ...
            if let cell = tableView.cellForRow(at: indexPath) as? SongTableViewCell {
                ...
                let actionInfo = getActionDetails(from: SongAction.show(title: cell.nameLabel.text, context: infoContext(from: indexPath, filtering: container?.filterContainer != nil)), indexPath: indexPath, vc: container?.filterContainer ?? container as? UIViewController, useAlternateTitle: true)
                
                let goTo = SwipeAction.init(style: .default, title: actionInfo?.details.title, handler: { _, _ in actionInfo?.details.handler() }, image: actionInfo?.action.icon)
            ...
            if let _ = container?.filterContainer {
                ...
                let context = SwipeAction.init(style: .default, title: details?.details.title.lowercased(), handler: { _, _ in details?.details.handler() }, image: details?.action.icon)
        ...
        switch orientation {
            ...
            case .left:
                ...
                if musicPlayer.nowPlayingItem != nil, let cell = searchViewController.tableView.cellForRow(at: indexPath) as? SongTableViewCell {
                    ...
                    let actionInfo = searchViewController.getActionDetails(from: .queue(name: cell.nameLabel.text, query: nil), indexPath: indexPath, actionable: searchViewController, vc: searchViewController, entityType: .song, entity: searchViewController.getEntity(at: indexPath)!)
                    
                    let queue = SwipeAction.init(style: .default, title: actionInfo?.details.title.lowercased(), handler: { _, _ in actionInfo?.details.handler() }, image: actionInfo?.action.icon)
            ...
            case .right:
                ...
                if let cell = searchViewController.tableView.cellForRow(at: indexPath) as? SongTableViewCell {
                    ...
                    let actionInfo = searchViewController.getActionDetails(from: .show(title: cell.nameLabel.text, context: searchViewController.context(from: indexPath)), indexPath: indexPath, actionable: searchViewController, vc: searchViewController, entityType: searchViewController.sectionDetails[indexPath.section].category.entity, entity: searchViewController.getEntity(at: indexPath)!, useAlternateTitle: true)
                        
                    let goTo = SwipeAction.init(style: .default, title: actionInfo?.details.title.lowercased(), handler: { _, indexPath in actionInfo?.details.handler() }, image: actionInfo?.action.icon)
        ...
        switch orientation {
            ...
            case .right:
                ...
                if let cell = queueViewController.tableView.cellForRow(at: indexPath) as? SongTableViewCell {
                    ...
                    let actionInfo = queueViewController.getActionDetails(from: .show(title: cell.nameLabel.text, context: queueViewController.context(from: indexPath)), indexPath: indexPath, actionable: queueViewController, vc: queueViewController, entityType: .song, entity: queueViewController.getSong(from: indexPath)!)
                
                    let goTo = SwipeAction.init(style: .default, title: "show...", handler: { _, indexPath in actionInfo?.details.handler() }, image: actionInfo?.action.icon)
