...
extension SwipeTableViewCell {
    ...
    open override var accessibilityCustomActions: [UIAccessibilityCustomAction]? {
        get {
            ...
            let leftActions = swipeDelegate?.tableView(tableView, editActionsForRowAt: indexPath, for: .left) ?? []
            let rightActions = swipeDelegate?.tableView(tableView, editActionsForRowAt: indexPath, for: .right) ?? []
