...
class AlbumItemsViewController: UIViewController, FilterContextDiscoverable, SupplementaryHeaderInfoLoading, QueryUpdateable, CellAnimatable, SingleItemActionable, PillButtonContaining, Refreshable, IndexContaining, AlbumArtistTransitionable, ArtistTransitionable, GenreTransitionable, ComposerTransitionable, EntityVerifiable, TopScrollable, EntityContainer {
    ...
    @objc lazy var headerView: HeaderView = {
        ...
        view.buttonDetails = [
            ...
            (.artist, #imageLiteral(resourceName: "Artists15"), album?.representativeItem?.validAlbumArtist, { [weak self] in self?.goToArtist() }),
            (.info, #imageLiteral(resourceName: "InfoNoBorder13"), nil, { [weak self] in self?.entityVC?.showOptions() }),
            (.affinity, SecondaryCategory.loved.propertyImage(from: album, context: .header), nil, { [weak self] in self?.setLiked() }),
    ...
    var applicableActions: [SongAction] { return [SongAction.collect, .info(context: .album(at: 0, within: [])), .queue(type: .all, name: album?.representativeItem?.albumTitle ??? .untitledAlbum, query: nil), .queue(type: .playNext, name: album?.representativeItem?.albumTitle ??? .untitledAlbum, query: nil), .queue(type: .playLater, name: album?.representativeItem?.albumTitle ??? .untitledAlbum, query: nil), .newPlaylist, .addTo] }
