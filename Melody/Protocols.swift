...
extension Detailing {
    ...
    func getActionDetails(from action: SongAction, indexPath: IndexPath, actionable: SingleItemActionable?, vc: UIViewController?, entityType: Entity, entity: MPMediaEntity, useAlternateTitle alternateTitle: Bool = false) -> (details: ActionDetails, action: SongAction)? {
        
        guard let actionable = actionable, let vc = vc else { return nil }
        
        return (actionable.singleItemActionDetails(for: action, entity: entityType, using: entity, from: vc, useAlternateTitle: alternateTitle), action)
    }
}
